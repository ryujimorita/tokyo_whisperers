...
class EpochProgressCallback(TrainerCallback):
    ...
    def on_epoch_begin(self, args: TrainingArguments, state: TrainerState, control, **kwargs):
        ...
        if state.is_world_process_zero:  # only show progress bar on main process
            ...
            self.epoch_pbar = tqdm(
                total=args.num_update_steps_per_epoch,
                desc=desc,
                leave=True,
                position=0
            )
            self.completed_steps = (self.current_epoch - 1) * args.num_update_steps_per_epoch
    ...
    def on_step_end(self, args: TrainingArguments, state: TrainerState, control, **kwargs):
        ...
        if state.is_world_process_zero and self.epoch_pbar is not None:
            ...
            global_progress = (self.completed_steps + self.epoch_pbar.n) / self.total_steps * 100
